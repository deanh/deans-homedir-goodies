...
;; $URL: svn://rubyforge.org/var/svn/emacs-rails/branches/rinari-merge/rails-controller-layout.el $
;; $Id: rails-controller-layout.el 197 2007-07-18 00:35:13Z technomancy $
...
(defun rails-controller-layout:keymap (type)
  (let* ((name (capitalize (substring (symbol-name type) 1)))
    ...
    (when type
      ...
      (define-keys map
        ("\C-c g" 'rails-controller-layout:switch-to-migration)
        ("\C-c m" 'rails-controller-layout:switch-to-model)
        ("\C-c h" 'rails-controller-layout:switch-to-helper)
        ("\C-c f" 'rails-controller-layout:switch-to-functional-test)
        ("\C-c u" 'rails-controller-layout:switch-to-unit-test)
        ("\C-c c" 'rails-controller-layout:switch-to-controller)
